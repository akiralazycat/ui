...
export function AvailabilityLab() {
  ...
  const snippet = useMemo(
    () => `<Availability\n  targets={targets}\n  theme=\"${theme}\"\n  mode=\"${mode}\"\n  presentation=\"${presentation}\"${region === "CN" ? '\n  region="CN"' : ""}\n/>`,
    [mode, presentation, region, theme],
  );
  ...
  return (
    <section className="lab" id="availability">
      ...
      <div className="notes">
        ...
        <article><span>C</span><h3>Region is separate.</h3><p>China-specific or regional distribution can be declared per target without pretending that a store and an operating system are the same thing.</p></article>
      </div>
    </section>
  );
}
